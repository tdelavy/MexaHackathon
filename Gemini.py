...
if uploaded_file is not None:
    ...
    if recognized_text:
        ...
        if not st.session_state['chat_locked']:
            ...
            reports_path = os.path.join(current_dir, "Reports")  # Relative path to the "Reports" folder

            # Function to open the folder in the system's file explorer
            def open_folder():
                if platform.system() == "Windows":
                    os.startfile(reports_path)  # Windows
                elif platform.system() == "Darwin":  # macOS
                    subprocess.run(["open", reports_path], check=True)
